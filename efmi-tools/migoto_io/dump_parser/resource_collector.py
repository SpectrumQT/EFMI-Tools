...
from .calls_collector import ShaderMap, Slot, CallsCollector, ShaderCallBranch, BranchCall
...
class ResourceCollector:
    ...
    def collect_branch_call_resource(self, branch_call: BranchCall, resource_tag, source: Source, layout: BufferLayout):
        ...
        descriptor: Optional[ResourceDescriptor] = branch_call.call.get_filtered_resource(filter_attributes)
        ...
        resource_hash = descriptor.hash if source.fast_hash else descriptor.get_sha256()

        cache_id = (resource_hash, source.file_ext)

        cached_resource = self.cache.get(cache_id, None)

        if cached_resource is None:
            try:
                resource = WrappedResource(descriptor, load_header=source.parse_header)
                if layout is not None:
                    resource.load_buffer(layout)
                self.cache[cache_id] = resource
            except Exception as e:
                raise e
        else:
            cached_resource.bind_descriptor(descriptor, allow_conflicts=[
                ResourceConflict.OldHash, ResourceConflict.SlotId, ResourceConflict.SlotShaderType, ResourceConflict.SlotType
            ], error_on_conflict=True, load_header=source.parse_header)
            resource = cached_resource  

        if resource.ext in ['buf', 'txt']:
            branch_call.resources[resource_tag] = resource
        else:
            branch_call.textures.append(resource)
