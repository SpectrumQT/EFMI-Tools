...
class Version:
    def __init__(self, efmi_ini_path):
        self.efmi_ini_path = efmi_ini_path
        self.version = None
        self.parse_version()

    def parse_version(self):
        with open(self.efmi_ini_path, "r") as f:
            ...
            for line in f.readlines():
                ...
                if len(result) != 3:
                    raise ValueError(f'Malformed EFMI Tools version!')

                self.version = result

                return

        raise ValueError(f'Failed to locate EFMI Tools version!')
...
class Project:
    def __init__(self):
        ...
        self.efmi_tools_dir = self.root_dir / 'efmi-tools'
        self.release_dir = self.root_dir / '!RELEASES'
        self.efmi_tools_init_path = self.efmi_tools_dir / '__init__.py'
        self.version = Version(self.efmi_tools_init_path)
    ...
    def build(self):
        ...
        release_path = self.version_dir / f'EFMI-Tools'
        shutil.copytree(self.efmi_tools_dir, release_path, ignore=shutil.ignore_patterns('__pycache__', '.idea', 'efmi-tools_updater'))
